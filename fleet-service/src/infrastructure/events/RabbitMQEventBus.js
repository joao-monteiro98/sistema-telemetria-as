// fleet-service/src/infrastructure/events/RabbitMQEventBus.js
...
class RabbitMQEventBus extends IEventBus {
    ...
    async init() {
        ...
        while (retries > 0) {
            try {
                ...
                // Garantimos que a exchange do tipo topic existe
    ...
    publish(eventName, payload, correlationId = 'N/A') {
        ...
        // Publicar para a exchange com a routing key igual ao nome do evento
    ...
    async subscribe(eventName, handler) {
        ...
        // Criamos uma fila exclusiva para este serviço e este evento para assegurar durabilidade
        ...
        this.channel.consume(queueName, async (msg) => {
            if (msg !== null) {
                ...
                try {
                    ...
                    const correlationId = content.correlationId || 'N/A';
                    
                    // Executamos o handler do Core
                    ...
                    // Em caso de erro, removemos da fila para evitar bloqueio infinito (DLQ lógica manual ou simplesmente descartar)
